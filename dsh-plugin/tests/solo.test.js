 ...
 * plugin is installed: both read the same REINS_HOME, register at the Relay as
 ...
 * heartbeats publishing into a shared REINS_HOME, which would race the
...
import { readRuntime, writeRuntime } from '@reins/bridle'
...
test('the plugin refuses an identity another process is already serving', async (t) => {
  ...
  assert.match(errors[0], /REINS_HOME/u, 'the error does not say how to run two on purpose')

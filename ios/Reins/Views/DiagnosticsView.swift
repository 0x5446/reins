...
struct DiagnosticsView: View {
            ...
            Group {
                if session.notes.isEmpty {
                    Placeholder(
                        icon: "waveform.path",
                        title: "Nothing yet",
                        detail: "Lines appear here each time Reins dials this Mac."
                    )
                } else {
                    ScrollViewReader { scroller in
                        List {
                            Section {
                                ...
                                Text("Addresses, verdicts, and timings only — never a key, a folder, or anything from a conversation. Kept in memory, so it is gone when Reins closes.")
                            }
                        }
                        .listStyle(.plain)
    ...
    private func copy() {
        ...
        let body = session.notes
            ...
            .joined(separator: "\n")
        #if canImport(UIKit)
        UIPasteboard.general.string = "Reins \(session.machine.name)\n\(body)"
        #endif
        copied = true
        Task {

    ...
    private var install: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Metrics.gutter) {
                ...
                Text("Open Terminal and paste this. It installs Bridle, the small helper that connects Reins to the agent already running on that machine.")
                    .font(.system(size: 15))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var scan: some View {
        ScannerView { payload in
            guard let bundle = try? Pairing.decodeLink(payload) else {
                problem = "That isn’t a Reins pairing code."
                return false
            }
